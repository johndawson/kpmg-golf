...
var s_account="kpmgllpphilprod,kpmgglobal"
...
s.linkInternalFilters="javascript:,kpmg"
...
s.events=s.apl(s.events,'event2',',',1);//pageview event

if(!s.prop7)s.prop7= s.getQueryParam('q')
...
if(url)
{
 var s_templtv
 var s_templtve
 if(s.linkTrackVars)s_templtv=s.linkTrackVars
 if(s.linkTrackEvents)s_templtve=s.linkTrackEvents
 s.tempevents=s.events
 s.tempprp17=s.prop17
 s.tempevar20=s.eVar20
  
 s.linkTrackVars="events,prop17,eVar20,prop48,prop49,prop50,eVar48,eVar49,eVar50"
...
if(s.templtve)s.linkTrackevents=s.templtve
if(s.tempevents)s.events=s.tempevents
if(s.tempprp17)s.prop17=s.tempprp17
if(s.tempevar20)s.eVar20=s.tempevar20
}
...
if(s.campaign)s.events=s.apl(s.events,'event5',',',1);


if(!s.eVar13)s.eVar13=s.getQueryParam('icid');
s.eVar13=s.getValOnce(s.eVar13,'s_eVar13',30);
if(s.eVar13)s.events=s.apl(s.events,'event6',',',1);

if(!s.prop15)// campiagn and campaign pathing
{
if(s.campaign)s.tempprp15=s.campaign
s.prop15=s.getAndPersistValue(s.tempprp15,'s_prop15',0)
if(!s.prop16 && s.prop15)
  {
        s.prop16=s.prop15+' : '+s.pageName
   }
}

if(s.prop1 && !s.eVar1)s.eVar1=s.prop1//Sub section
...
if(s.prop5 && !s.eVar5)s.eVar5=s.prop5//content title
if(s.prop6 && !s.eVar6)s.eVar6=s.prop6//content id

if(!s.eVar11)
{
 s.eVar11=(s.pageName)?s.pageName:document.URL
}
...
// time parting
s.prop8=s.eVar8=s.getTimeParting('h','-5',year); // Set hour 
s.prop9=s.eVar9=s.getTimeParting('d','-5',year); // Set day
s.prop10=s.eVar10=s.getTimeParting('w','-5',year); // Set Weekend / Weekday
if(s.prop7 || s.prop11)
        {
           if(s.prop7)
           {
           ...
           if(s.prop11)
           {
           s.prop11.toLowerCase()
           s.eVar14=s.prop11
           }
           
        if(s.prop12)s.eVar15=s.prop12
               
               /* Set de-duped onsite search event */
               var t_search=s.getValOnce(s.eVar7,'ev7',0)
               var t_search2=s.getValOnce(s.eVar14,'ev14',0)
               if(t_search || t_search2)
...
s.visitorNamespace="kpmgllp1"
s.trackingServer="kpmgllp1.122.2o7.net"
